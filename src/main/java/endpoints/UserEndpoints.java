...
import models.UserOperationResponse;
...
public class UserEndpoints {
    ...
    public static Response getUserNotFound(int userID){
        return given().spec(RequestSpec.getRequestSpec())
                .pathParam("id",userID)
                .when().get(ReqresEndpoints.USER_BY_ID)
    ...
    public static Response updateUser(UserData payLoad,int userID){
        return given().spec(RequestSpec.getRequestSpec())
                .pathParam("id",userID)
                .body(payLoad)
    ...
    public static Response patchUser(UserData payLoad,int userID){
        return given().spec(RequestSpec.getRequestSpec())
                .pathParam("id",userID)
                .body(payLoad)
    ...
    public static Response deleteUser(int userID){
        return given().spec(RequestSpec.getRequestSpec())
                ...
                .then().spec(ResponseSpec.getResponseSpec(204))
                .extract().response();
    }
    public static Response delayResponse(int delay){
        return given().spec(RequestSpec.getRequestSpec())
                .queryParam("delay",delay)
                .when().get(ReqresEndpoints.DELAYED_RESPONSE)
                .then().spec(ResponseSpec.getResponseSpec(200))
                .extract().response();
    }
}
